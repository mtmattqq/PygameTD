...
def find_path(map=[[]]):
    ...
    def dfs(pos_now=[]):
        ...
        for d in MOVEMENT:
            ...
            if (
                ...
                path.append(
                    transform(vec2D(pos_now[1], pos_now[0]), tile.TILE_SIZE))
    ...
    if dfs(enemy_source):
        path.append(
            transform(vec2D(main_tower[1], main_tower[0]), tile.TILE_SIZE))
        return path
    else:
        return None
...
def level(level_now='basic_level.json'):
    ...
    if wave >= 100:
        ...
        enemy_types[0] = enemy.angry_basic(
    ...
    if wave >= 150:
        ...
        enemy_types[2] = enemy.super_shield(
    ...
    if wave >= 200:
        ...
        enemy_types[0] = enemy.shielded_basic(
            enemy_types[0].pos, 0, 0, 0, 10, [enemy_types[0].pos])
        enemy_types[0].location = enemy_types[0].pos
    if wave >= 225:
    ...
    if wave >= 250:
        ...
        enemy_types[2] = enemy.ultra_shield(
            enemy_types[2].pos, 0, 0, 0, 10, [enemy_types[2].pos])
        enemy_types[2].location = enemy_types[2].pos
        ...
        if game_timer > send_next_wave:
            ...
            if wave >= 200:
                ...
                enemy_types[0] = enemy.shielded_basic(
                    enemy_types[0].pos, 0, 0, 0, 10, [enemy_types[0].pos])
                enemy_types[0].location = enemy_types[0].pos
                boss_types[0] = enemy.shielded_basic_boss(
            ...
            if wave >= 225:
                ...
                enemy_types[1] = enemy.storm_eye(
                    enemy_types[1].location, 0, 0, 0, 10, [enemy_types[1].pos])
                enemy_types[1].location = enemy_types[1].pos
                boss_types[1] = enemy.storm_eye_boss(
            ...
            if wave >= 250:
                ...
                enemy_types[2] = enemy.ultra_shield(
                    enemy_types[2].pos, 0, 0, 0, 10, [enemy_types[2].pos])
                enemy_types[2].location = enemy_types[2].pos
                boss_types[2] = enemy.ultra_high_armor_boss(
                    ...
                    if (
                        ...
                        if ct == 1:
                            new_tower = tower.basic_tower(
                                vec2D(selected_tile[1], selected_tile[0]), volume)
                        elif ct == 2:
